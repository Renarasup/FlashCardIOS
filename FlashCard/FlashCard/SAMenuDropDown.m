...
- (void)uiSetUp
{
    ...
    _tableMenu.backgroundColor = [UIColor grayColor];
...
{
    return 50;//(_rowHeight > 0.0) ? _rowHeight : kSACellHeight;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(itemData.itemName != nil){
        ...
        //if(IPAD) cell.textLabel.font = [UIFont boldSystemFontOfSize:25.0];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MenuItemData *itemData = (MenuItemData *)[_menuDataSource objectAtIndex:indexPath.row];
    if([_delegate respondsToSelector:@selector(saDropMenu:didClickedAtIndex: didClickItem:)]) {
        [_delegate saDropMenu:self didClickedAtIndex:indexPath.row didClickItem:itemData.itemName];
    }
